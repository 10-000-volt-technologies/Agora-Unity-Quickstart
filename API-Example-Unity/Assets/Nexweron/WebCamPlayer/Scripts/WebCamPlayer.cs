namespace Nexweron.WebCamPlayer
{
    ...
    public class WebCamPlayer : MonoBehaviour
	{
		...
		private void Update() {
            agoraTexture = null;
            newTexture = null;
            if (FindObjectOfType<agora_gaming_rtc.VideoSurface>()) {
                agoraTexture = FindObjectOfType<agora_gaming_rtc.VideoSurface>().nativeTexture;
            }

            if (FindObjectOfType<AgoraScreenShare>()) {
                newTexture = FindObjectOfType<AgoraScreenShare>().renderTexture;
            }
            newTexture.DiscardContents();

            Graphics.Blit(agoraTexture, newTexture);
